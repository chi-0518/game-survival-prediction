...
def load_data():
    ...
    sentiment_data = pd.DataFrame(list(db.disable_game_allcomment_notrepeat_04302025.find({}, {"遊戲ID": 1, "評論內容": 1, "推薦狀態": 1, "_id": 0})))
    adapter_data = pd.DataFrame(list(db.disable_game_year_news_5000_0525.find({}, {"遊戲ID": 1, "owners_estimate": 1, "news_count": 1, "average_forever": 1, "median_forever": 1, "_id": 0})))
    ...
    for col in ['owners_estimate', 'news_count', 'average_forever', 'median_forever']:
        merged_data[col] = np.log1p(merged_data[col].fillna(0))
...
def prepare_data(tokenizer, texts, labels, adapter_feats):
    ...
    return TensorDataset(encodings['input_ids'], encodings['attention_mask'], torch.tensor(labels), torch.tensor(adapter_feats, dtype=torch.float32))
